    ...
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"0"}' label="菜品分类" prop="caipinfenlei">
            <el-input v-model="ruleForm.caipinfenlei" 
                placeholder="菜品分类" clearable ></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('caipinfenlei/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
                  ...
                  params: params
                     ...
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('caipinfenlei/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
